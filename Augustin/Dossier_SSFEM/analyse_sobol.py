...
def calculer_sobol():
    ...
    # 1. execution de la chaine NISP jusqu'a la regression
    nisp = NISP_RFF(D, P, N)
    ...
    # l'onde 'j' est controlee par les variables aleatoires 2*j (cos) et 2*j+1 (sin)
    for p_idx, alpha in enumerate(multi_indices):
        ...
        if coeff_carre > 1e-12:
            # pour chaque onde D, on verifie si elle est impliquee dans ce polynome
            for j in range(D):
                idx_cos = 2 * j
                idx_sin = 2 * j + 1
                
                # si l'une des variables de l'onde est dans le polynome, on ajoute la contribution
                if alpha[idx_cos] > 0 or alpha[idx_sin] > 0:
                    sobol_totaux_par_onde[j] += coeff_carre / variance_totale

    # 4. trace du spectre de sensibilite
    plt.figure(figsize=(12, 6))
    
    ondes_indices = np.arange(1, D + 1)
    
    # on met en evidence les ondes les plus influentes
    ...
    print("\n Histogramme de Sobol généré : sobol_indices.png")
