...
#d'un cylindre, avec module de Young tiré d'une loi normale puis convolué
...
class ef_mc:
    ...
    def __init__(self):
        ...
        self.pp_filtrage_gaussien = gfc.reqPP("applique_filtre_uniforme")     
        

    def monte_carlo(self,n_mc):
        #Méthode monte Carlo
        #A chaque itération, il faut retirer E aléatoirement
        #puis appliquer le filtre gaussien
    ...
    def critere_cov(self,limite=0.01,niter=200):
    ...
    def convergence_max_var(self,niter=50, indice=366):
        #Méthode de convergence de la simulation
        #niter : nombre d'itérations
        #indice : indice du noeud pour lequel on veut observer la convergence
        #366 est le noeud de maximum de variance

        self.convergence_mc = []
        for i in range(niter):
            self.monte_carlo(1)
            self.traitement()
            self.convergence_mc.append(self.moyenne_norme_u[366])
